...
function getLanguagePipeline() {
  var lang = document.documentElement.lang;
  if (lang !== "en" && window.lunr && window.lunr[lang]) {
    return {
      stemmer: window.lunr[lang].stemmer,
      trimmer: window.lunr[lang].trimmer,
      stopWordFilter: window.lunr[lang].stopWordFilter
    };
  }
  return {
    stemmer: elasticlunr.stemmer,
    trimmer: elasticlunr.trimmer,
    stopWordFilter: elasticlunr.stopWordFilter
  };
}
...
function makeTeaser(body, terms) {
  ...
  var pipeline = getLanguagePipeline();

  var stemmedTerms = terms.map(function (w) {
    return pipeline.stemmer(w.toLowerCase());
  });
  var termFound = false;
  var index = 0;
    ...
    for (var j in words) {
      ...
      if (word.length > 0) {
        for (var k in stemmedTerms) {
          if (pipeline.stemmer(word).startsWith(stemmedTerms[k])) {
            value = TERM_WEIGHT;
            termFound = true;
          }
        }
...
function formatSearchResultItem(item, terms, index) {
  var doc = item.doc || index.documentStore.getDoc(item.ref);
  if (!doc) {
    return (
      '<div class="item">' +
  ...
  return (
    '<div class="item">' +
    `<a href="${item.ref}">${doc.title || item.ref}</a>` +
    `<span>${makeTeaser(doc.body || "", terms)}</span>` +
    "</div>"
  );
}
...
function initSearch() {
  ...
  var initIndex = async function () {
    if (index === undefined) {
      if (typeof window.searchIndex !== "undefined") {
        ...
        // Since we use elasticlunr_javascript format, we need to load the js file and then use window.searchIndex
        await new Promise((resolve, reject) => {
          let script = document.createElement('script');
          script.src = `/search_index.${document.documentElement.lang}.js`;
      ...
      index.pipeline.add(pipeline.trimmer, pipeline.stopWordFilter, pipeline.stemmer);
    }
    return index;
  };
  ...
  searchInput.addEventListener(
    ...
    debounce(async function () {
      ...
      var searchIdx = await initIndex();
      var results = searchIdx.search(term, options);
      if (results.length === 0) {
